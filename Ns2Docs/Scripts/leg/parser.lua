...
local type = type
local throw = throw
local RegisterError = RegisterError
...
local function CHECK(rule, desc)
  ...
  return patt + m.P(function (s, i)
    ...
    local vicinity = s:sub(i-15, i+15)
	error('Malformed '..desc..' in line '.. vicinity .. ': ' ..rule:lower()..'" is missing!' .. i)
...
rules = {
  ...
  , NumericFor  = m.V'FOR' *S* m.V'Name' *S* m.V'=' *S* m.V'Exp' 
	...
	, Class = 'class' *S* m.V'ClassName' * (S * m.V'(' * m.V'SuperClass' * m.V')')^-1
	, PrepareMixin = 'PrepareClassForMixin' *S* '(' *S* m.V'MixinID' *S* ',' *S* m.V'MixinID' *S* ')'
  ...
  -- solving the left recursion problem
	...
	, ParList  = listOf(m.V'Name', S* m.V',' *S) * (S* m.V',' *S* '...')^-1
	           + '...'
