...
local comment_block = #(m.P'/*') * function (subject, i1)
	local level = _G.assert( subject:match('^/%*', i1) )
	local _, i2 = subject:find('*/', i1+2, true)  -- true = plain "find substring"
	return (i2 and (i2+1)) or error('unfinished long brackets')(subject, i1)
end
...
local multi  = (m.P'--' * long_brackets) + comment_block
...
do
	-- TEMP ddd limites: local ddd  = m.P'\\' * (N^3 + N^2 + N^1)
	          --  OPEN  and  (   (\?)    or ( anything not CLOSE or \n )^0 ) and    CLOSE
	local Str1 = m.P'"' * IDENTIFIER * (m.P'"' + error'unfinished string')
	local Str2 = m.P"'" * IDENTIFIER * (m.P"'" + error'unfinished string')
	CLASSNAME = Str1 + Str2
end
