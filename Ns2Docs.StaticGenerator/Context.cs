	...
	public class Context
	{
		...
		public object this[string key]
		{
			get 
            {
                ...
                {
                    TT += (DateTime.Now - s).TotalSeconds;
                }
            }
        ...
        public static Dictionary<Type, Dictionary<string, PropertyInfo>> ps = new Dictionary<Type, Dictionary<string, PropertyInfo>>();
		...
		private object Variable(string markup)
		{
            ...
            {*/
              //  parts = R.Scan(markup, Liquid.VariableParser).ToArray();
            //}
            dt6 += (DateTime.Now - s).TotalSeconds;
            string firstPart = parts[0];
            Match firstPartSquareBracketedMatch = null;
            if (markup[0] == '[')
            {
                firstPartSquareBracketedMatch = squareBracketed.Match(firstPart);
                dt5 += (DateTime.Now - s).TotalSeconds;
                if (firstPartSquareBracketedMatch.Success)
                {
                    firstPart = Resolve(firstPartSquareBracketedMatch.Groups[1].Value).ToString();
                    dt4 += (DateTime.Now - s).TotalSeconds;
                }
            }
            dt3 += (DateTime.Now - s).TotalSeconds;
			object @object;
            if ((@object = FindVariable(firstPart)) != null)
            {
                for (int i=1; i<parts.Length; i++)
				{
                    ...
                    if (forEachPart == "AllMixins")
                    {
                        var v = ((IIndexable)@object)["AllMixins"];
                    }
                    DateTime s1 = DateTime.Now;
                    Type type = @object.GetType();
                    PropertyInfo info = null;
                    if (ps.ContainsKey(type) && ps[type].ContainsKey(forEachPart))
                    {
                        info = ps[type][forEachPart];
                        @object = info.GetValue(@object, null);
                        dt1 += (DateTime.Now - s).TotalSeconds;
                        continue;
                    }
                    else
                    {
                        info = type.GetProperties().FirstOrDefault(p => p.Name == forEachPart);
                        if (info != null)
                        {
                            if (!ps.ContainsKey(type))
                            {
                                ps[type] = new Dictionary<string, PropertyInfo>();
                            }
                            ps[@object.GetType()][forEachPart] = info;
                            DateTime s2 = DateTime.Now;
                            @object = info.GetValue(@object, null);
                            dt2 += (DateTime.Now - s2).TotalSeconds;
                            continue;
                        }
                    }
            ...
            t2 += (DateTime.Now - s).TotalSeconds;
			return @object;
		}
